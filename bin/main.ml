...
type response = { item : info } [@@deriving yojson { strict = false }]

type ext_inf = { title : string; logo_link : string; play_link : string }
[@@deriving yojson { strict = false }]

type ext_m3u = { ext_inf_list : ext_inf list }
[@@deriving yojson { strict = false }]

let ext_inf_of_info info =
  let model_id = string_of_int info.modelId in
  let logo_link =
    "https://adultiptv.net/livecams/image.php?id=sc_" ^ model_id
  in
  let play_link =
    "https://adultiptv.net/livecams/stream.php?id=sc_" ^ model_id
  in
  { title = info.username; logo_link; play_link }
